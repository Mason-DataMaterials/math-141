...
distance of less than 2000 miles. The points can be made transparent by adding an argument to `qplot()` called `alpha`, which must be given a value between 0 and 1.
...
flights %>%
  ...
  filter(n > 20,
         avg_dist < 2000) %>%
  qplot(x = avg_dist, y = avg_delay, data = ., size = n, alpha = .05)
```
